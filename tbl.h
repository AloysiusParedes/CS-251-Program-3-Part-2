...
//   try to apply the ADT princdfples unless we have
//   a very good reason...
...
typedef struct {
  ...
  char *rowname; //to hold row name (string)
} Row;
...
typedef struct {
  ...
  char **colname; //to hold column labels (array of strings)
} Table;
...
*
*/
extern ROW_CMP cmp_rows(Row *a, Row *b);
